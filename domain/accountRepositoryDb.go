...
func (repo AccountRepositoryDb) Save(acc Account) (*Account, *errs.AppError) {
	query := "INSERT INTO accounts(customer_id, opening_date, account_type, amount, status)values(?,?,?,?,?)"
	logger.Info("Account: " + acc.CustomerId)
	result, err := repo.client.DB.Exec(query, acc.CustomerId, acc.OpeningDate, acc.AccountType, acc.Amount, acc.Status)
	if err != nil {
	...
	if err != nil {
		...
		logger.Error("Error while getting last insert id fr new account: " + err.Error())
		return nil, errs.NewUnexpectedError("Unexpected error from database")
	}
